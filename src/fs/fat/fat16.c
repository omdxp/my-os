...
void fat16_to_proper_string(char **out, const char *in, size_t size)
{
	...
	while (*in != 0x00 && *in != 0x20)
	{
		...
		*out += 1;
		in += 1;
		// cannot process anymore since we have exceeded input buffer size
		if (i >= size - 1)
		{
			break;
		}
...
struct fat_directory *fat16_load_directory(struct disk *disk, struct fat_directory_item *item)
{
	int res = 0;
...
struct fat_item *fat16_find_item_in_directory(struct disk *disk, struct fat_directory *directory, const char *name)
{
	...
	for (int i = 0; i < directory->total; i++)
	{
		...
		{
			// found the file, creating a new fat_item
